...
parser.add_argument(
    ...
    default=r"C:\Users\kaden\Main\EE465\Watermarking_Project1\resutls\test_stegastamp_AGANF"
)
parser.add_argument(
    "--fingerprint_length",
    type=int,
    ...
    for i_epoch in range(args.num_epochs):
        ...
        for images, _ in tqdm(dataloader):
            ...
            if global_step % 3000 == 0:
                ...
                torch.save(
                    encoder.state_dict(),
                    join(CHECKPOINTS_PATH, EXP_NAME + f"_{global_step}_encoder.pth"),
                )
                torch.save(
                    decoder.state_dict(),
                    join(CHECKPOINTS_PATH, EXP_NAME + f"_{global_step}_decoder.pth"),
                )
                torch.save(
                    decoder.state_dict(),
                    join(CHECKPOINTS_PATH, EXP_NAME + f"_{global_step}_decoder.pth"),
                )
                f = open(join(CHECKPOINTS_PATH, EXP_NAME + f"_{global_step}_variables.txt"), "w")
